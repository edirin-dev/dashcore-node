...
/**
 * Provides a friendly event driven API to vkaxd in Node.js. Manages starting and
 * stopping vkaxd as a child process for application support, as well as connecting
 * to multiple vkaxd processes for server infrastructure. Results are cached in an
...
Dash.prototype._loadSpawnConfiguration = function(node) {
  ...
  $.checkArgument(this.options.spawn, 'Please specify "spawn" in vkaxd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in vkaxd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in vkaxd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './vkax.conf');

  log.info('Using Vkax config file:', configPath);
...
Dash.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of vkaxcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of vkaxcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of vkaxcore-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in vkax.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that vkaxd is undergoing a reindex. ' +
             ...
             'of vkaxcore-node services will start.');
    node._reindex = true;
  }
};
...
Dash.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === dashcore.Networks.testnet) {
    networkPath = 'testnet3/vkax.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/vkax.conf';
    }
  }
  return networkPath;
};
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Vkax Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Dash.prototype._checkReindex = function(node, callback) {
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('Vkax Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
...
Dash.prototype._stopSpawnedDash = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/vkaxd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned vkax process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean vkax process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Dash.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedDash(function(err) {
    ...
    log.info('Starting vkax process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Vkax process unexpectedly exited with code:', code);
        log.warn('Restarting vkax child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Vkax process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn vkaxd.'));
      }
...
Dash.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to vkaxd.'));
    }
...
Dash.prototype.start = function(callback) {
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Vkax configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {String|Address|Array} addressArg - An address string, vkaxcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
...
Dash.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    ...
    if (clone) {
      // reverse start and end as the order in vkaxcore is most recent to less recent
 ...
 * @param {Object} txid - A vkax transaction id
 * @param {Function} callback
 */
...
/**
 * Will retrieve a block as a vkaxcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
  ...
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('vkaxd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('vkaxd process did not exit'));
      }
